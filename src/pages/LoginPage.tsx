...
export function LoginPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      redirect.searchParams.set('next', safeNext(params.get('next') || '/groups'));
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-ink p-5">
      <section className="w-full max-w-md rounded-3xl bg-mist p-7 md:p-10">
        ...
        <h1 className="mt-2 font-display text-5xl">Good to see you.</h1>
        <p className="mt-3 text-sm leading-6 text-ink/65">
          {mode === 'password'
            ? 'Sign in with your email and password to see your trips.'
            : 'Sign in or create an account with a link sent to your email.'}
        </p>
        <form className="mt-8 space-y-4" onSubmit={submit} aria-busy={busy}>
          ...
          <Button className="w-full" disabled={busy || !isSupabaseConfigured}>
            {mode === 'password' ? (busy ? 'Signing in…' : 'Sign in') : (busy ? 'Sending…' : 'Send magic link')}
          </Button>
        </form>
        {message && <p className="mt-4 text-sm" role="status">{message}</p>}
        {error && <p className="mt-4 text-sm text-red-700" role="alert">{error}</p>}
        <div className="mt-5 text-center">
          {mode === 'password' && <p className="mb-2 text-xs leading-5 text-ink/65">New here, or don’t have a password? Use a magic link.</p>}
          <button type="button" onClick={switchMode} disabled={busy} className="rounded text-sm text-moss underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ember focus-visible:ring-offset-2 disabled:opacity-50">
...
function safeNext(path: string) {
  try {
    ...
    return parsed.origin === window.location.origin ? parsed.pathname + parsed.search + parsed.hash : '/groups';
  } catch {
    return '/groups';
  }
}
