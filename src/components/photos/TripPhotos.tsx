...
import { deletePhoto, errorMessage, loadTripPhotos, PHOTO_PAGE_SIZE, releasePhotos } from '../../lib/groups';
...
export function TripPhotos({ tripId, title }: { tripId: string; title: string }) {
  ...
  async function remove(id: string) {
    if (deleting.current || !window.confirm('Delete this photo permanently?')) return;
    deleting.current = true; setBusy(true); setDeleteError(''); setMessage('');
    try {
      await deletePhoto(id, user.id);
      setMessage('Photo deleted.');
      if (data?.photos.length === 1 && page > 0) setPage(value => value - 1);
      else reload();
  ...
  return <section className="mt-8" aria-labelledby="trip-moments-heading">
    ...
    <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
      {data?.photos.map((photo, index) => {
        ...
        return <div key={photo.id}>{photo.url ? <a href={photo.url} target="_blank" rel="noreferrer" aria-label={`Open ${label}`} className="block overflow-hidden rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ember">
          <img src={photo.url} alt={label} loading="lazy" className="aspect-square w-full object-cover transition hover:scale-105" />
        </a> : <div role="img" aria-label={`${label} is unavailable`} className="flex aspect-square items-center justify-center rounded-2xl bg-sand p-4 text-center text-sm text-ink/65">Photo unavailable</div>}
          <div className="mt-2 flex flex-wrap items-center justify-between gap-2">{!hasLocation(photo) && <span className="text-xs text-ink/60">No location</span>}{photo.uploaded_by === user.id && <Button className="ml-auto" size="sm" variant="ghost" disabled={busy} aria-label={`Delete ${label}`} onClick={() => void remove(photo.id)}>Delete</Button>}</div>
        </div>;
      })}
    </div>
